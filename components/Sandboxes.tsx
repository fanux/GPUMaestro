...
import { Plus, Terminal, Clock, ExternalLink, Search, Zap, StopCircle, RefreshCcw, Activity, Code2, Globe } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-white">Interactive Sandboxes</h1>
          <p className="text-slate-400">Pre-configured environments for Transformer debugging and model exploration.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        {[
          { name: 'LLM Debugging', framework: 'Transformers + Accelerate', color: 'indigo' },
          { name: 'Diffusion Studio', framework: 'Diffusers + PyTorch', color: 'purple' },
          { name: 'Graph RL', framework: 'DeepMind GraphNet', color: 'emerald' },
          { name: 'TinyML Lab', framework: 'TensorFlow Lite', color: 'amber' },
        ].map((tpl) => (
          <div key={tpl.name} className="p-4 bg-slate-900/40 border border-slate-800 rounded-2xl hover:border-slate-600 transition-all cursor-pointer group">
            <div className={`w-8 h-8 rounded-lg bg-${tpl.color}-500/10 text-${tpl.color}-400 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform`}>
              <Code2 size={16} />
            </div>
            <h4 className="text-sm font-bold text-white">{tpl.name}</h4>
            <p className="text-[10px] text-slate-500 mt-1">{tpl.framework}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-slate-900/40 border border-slate-800 rounded-2xl overflow-hidden backdrop-blur-sm">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
          <div className="flex items-center gap-3 bg-slate-950 px-4 py-2 rounded-lg border border-slate-800 w-full max-w-md">
            ...
            <input type="text" placeholder="Filter sandboxes..." className="bg-transparent border-none outline-none text-sm w-full" />
          </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-950/50 text-[10px] uppercase tracking-widest font-bold text-slate-500 border-b border-slate-800">
              <th className="px-6 py-4">Sandbox Name</th>
              <th className="px-6 py-4">Owner</th>
              <th className="px-6 py-4">GPU Resources</th>
              ...
              >
                ...
                <td className="px-6 py-5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-slate-300">{session.gpuRequested}x GPU Fragment</span>
                  </div>
                </td>
                <td className="px-6 py-5">
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="md:col-span-2 space-y-6">
                <div>
                  <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4">Live Logs</h3>
                ...
                <div className="bg-indigo-600/5 border border-indigo-500/20 rounded-2xl p-6">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="text-sm font-bold text-indigo-300 uppercase tracking-widest">AI Performance Insights</h3>
                  </div>
                  {isLoadingInsight ? (
                    <div className="flex items-center gap-4 text-slate-400 animate-pulse">
                      <RefreshCcw size={16} className="animate-spin" />
                      <p className="text-sm italic">Gemini is analyzing sandbox health...</p>
                    </div>
                  ) : (
                    <p className="text-sm leading-relaxed text-indigo-100/80">
                      {insight || "No insights available for this sandbox."}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-slate-800/40 rounded-xl border border-slate-700">
                      <p className="text-xs text-slate-500 mb-1">Resource Profile</p>
                      <p className="text-sm font-semibold text-slate-200">0.5 GPU / 8GB VRAM</p>
                    </div>
                    <div className="p-4 bg-slate-800/40 rounded-xl border border-slate-700">
                      <p className="text-xs text-slate-500 mb-1">Inactivity Timeout</p>
                      <p className="text-sm font-semibold text-slate-200">2 Hours</p>
                    </div>
                  </div>
                </div>
